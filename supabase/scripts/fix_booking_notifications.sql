-- =====================================================================
-- CORRECTION NOTIFICATIONS RÉSERVATIONS
-- À exécuter dans Supabase → SQL Editor (Run).
-- =====================================================================

-- Nouvelle réservation → chauffeur
...
-- Changement statut → passager (confirmé/refusé) ou chauffeur (annulation)
  ...
  if new.status = 'cancelled' and old.status in ('pending', 'confirmed') then
    ...
    if v_driver is not null then
      ...
      perform public.notify_user(
        ...
        jsonb_build_object('booking_id', new.id, 'trip_id', new.trip_id, 'reason', new.cancel_reason)
      );
    end if;
  end if;
