#!/usr/bin/env node
/**
 * Étape 3 — Placer le chauffeur à la destination (GPS simulé).
 *
 * Usage: npm run test:e2e:gps-dest
 */
import {
  CITIES,
  fetchOsrmRoute,
...
async function main() {
  ...
  const destName = state.toCity ?? "destination";

  console.log("══════════════════════════════════════════");
  console.log(` E2E 3/4 — Position chauffeur à ${destName}`);
  console.log("══════════════════════════════════════════\n");
  ...
  console.log(`\n📍 Position simulée: ${driver.lat}, ${driver.lng} (${destName})`);
